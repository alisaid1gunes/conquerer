...
export class AuthService {
  ...
  async loginUser(loginDto: LoginDto): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    const result = await this.userRepository.update(user.id, { refreshToken });
  ...
  async logoutUser(user: User): Promise<void> {
    ...
    if (!result) throw new Error('Logged out failed');
    await this.redisService.deleteKey(user.id.toString());
  }
  ...
  async changePassword(user: User, changePasswordDto: ChangePasswordDto): Promise<void> {
    ...
    const result = await this.userRepository.update(user.id, { password: hashedPassword, refreshToken: null });

    if (!result) throw new Error('Password could not be changed');
  ...
  async deleteUser(user: User): Promise<void> {
    ...
    await this.userRepository.softRemove(userWithRelations);
